    ...
    <div id="kt_app_content" class="app-content flex-column-fluid">
        ...
        <div id="kt_app_content_container" class="app-container container-fluid">
            ...
            <div class="row g-5 g-xl-10" id="reload_all">
                        ...
                        <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                            ...
                            <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
                                ...
                                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
                                    ...
                                    <!--end::Item-->
                                    <!--begin::Item-->
                                    <li class="breadcrumb-item text-muted">Pemantauan antrian penarikan</li>
                                    <!--end::Item-->
                                </ul>
                                <!--end::Breadcrumb-->
                            </div>
                ...
                <!--end::Toolbar-->
                <?php if(status_wd()) : ?>
                    <?php foreach(status_wd(99) AS $num => $title) : ?>
                    <div class="card mb-2 mb-xl-2">
                        <div class="card-header justify-content-center bg-light-<?= wd_color($num) ?> border-0 mt-5">
                                ...
                                <table class="table table-bordered table-row-gray-300 align-middle gs-0 gy-4">
                                    ...
                                    <thead>
                                        <tr class="fw-bold text-muted">
                                            <th class="min-w-100px"><center>Waktu</center></th>
                                            <th class="min-w-200px"><center>Customer</center></th>
                                            <th class="min-w-100px"><center>Nominal Penarikan</center></th>
                                            <th class="min-w-100px"><center>Nominal Diterima</center></th>
                                            <th class="min-w-200px"><center>Tujuan Penarikan</center></th>
                                            <?php if(in_array($num,[0])) : ?>
                                            <th class="min-w-100px text-center">Aksi</th>
                                            <?php elseif(in_array($num,[1])) : ?>
                                                <th class="min-w-100px text-center">Bukti TF</th>
                                            <?php elseif(in_array($num,[2])) : ?>
                                                <th class="min-w-100px text-center">Alasan</th>
                                            <?php endif;?>
                                        </tr>
                                    </thead>
                                    <!--end::Table head-->
                                    ...
                                    <tbody>
                                            ...
                                            <?php $no = 1; foreach($result[$num] AS $row) : ?>
                                                <tr>
                                                    ...
                                                    <td class="text-muted fw-semibold"><center><?= price_format($row['nominal_diterima'],1); ?></center></td>
                                                     <td>
                                                        <div class="d-flex align-items-center justify-content-center flex-column">
                                                            <div class="me-5" style="background-position: center; background-size: contain; background-repeat: no-repeat; width: 80px; height: 80px; background-image: url('<?= image_check($row['gambar_bank'], 'bank') ?>')"></div>
                                                            <span class="text-muted">Fee : <?= ifnull(price_format($row['fee'],1),' - '); ?></span>
                                                            <span class="text-muted">Nm.Rek : <?= $row['rekening'];?></span>
                                                            <span class="text-muted">No.Rek : <?= $row['nomor_rekening'];?></span>
                                                        </div>
                                                    </td>
                                                    <?php if(in_array($num,[0])) : ?>
                                                    <td>
                                                        <div class="d-flex justify-content-center flex-shrink-0">
                                                                ...
                                                                <button type="button" class="btn btn-icon btn-primary btn-sm me-1" title="Terima" onclick="status_penarikan(<?= $row['id_penarikan'] ?>,<?=$num;?>,1,'<?= '#'.$row['kode_penarikan'];?>')">
                                                                    <i class="ki-outline ki-check fs-2"></i>
                                                                </button>
                                                                <button type="button" class="btn btn-icon btn-danger btn-sm me-1" title="Batalkan" onclick="status_penarikan(<?= $row['id_penarikan'] ?>,<?= $num; ?>,2,'<?= '#'.$row['kode_penarikan'];?>')">
                                                    ...
                                                    <?php elseif(in_array($num,[2])) : ?>
                                                        <td>
                                                            <div class="d-flex justify-content-center flex-shrink-0">
                                                            <button class="btn btn-icon <?= ($row['alasan'] == '') ? 'btn-danger text-white' : 'btn-secondary' ?> btn-sm me-1" onclick="alert_show(this,'','<?= $row['alasan']; ?>')" <?= ($row['alasan'] == '') ? 'disabled' : '' ?>>
                                        ...
                                        <?php else : ?>
                                            <tr>
                                                <td colspan="7"><center>Tidak ada data dengan status <?= status_wd($num) ?></center></td>
                                            </tr>
                                        <?php endif;?>
                                    </tbody>
                                    <!--end::Table body-->
